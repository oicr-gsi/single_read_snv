...
input {
  ...
  Int override_memory_gb_CreateFeatureMap = 4
...
meta {
    ...
    WDL_AID: { exclude: [
        ...
        "CreateFeatureMap.jobMemory",
        "CreateFeatureMap.interval_list",
        "CreateFeatureMap.model_files",
        "CreateReport.cpus",
        "CreateReport.memory_gb",
        "PrepareRawFeatureMap.jobMemory",
        "PrepareRandomSampleFeatureMap.jobMemory",
...
parameter_meta {
  ...
  call SRSNVTasks.CreateFeatureMap {
    input:
      ...
      monitoring_script = monitoring_script,
      jobMemory = override_memory_gb_CreateFeatureMap,
  ...
  if (can_train_model && (!use_pre_trained_model)) {
    ...
    call SRSNVTasks.PrepareFeatureMapForTraining as PrepareRawFeatureMap {
        input:
            ...
            jobMemory = memory_gb_PrepareRawFeatureMap,
            monitoring_script = monitoring_script
    }
    
    # Prepare the random sample featuremap for training
    Int memory_gb_PrepareRandomSampleFeatureMap = select_first([override_memory_gb_PrepareRandomSampleFeatureMap, 128])
    ...
    call SRSNVTasks.PrepareFeatureMapForTraining as PrepareRandomSampleFeatureMap {
        input:
            ...
            read_filters_with_max_coverage = read_filters_with_max_coverage_,
            docker = global.ugbio_featuremap_docker,
            jobMemory = memory_gb_PrepareRandomSampleFeatureMap,
            monitoring_script = monitoring_script
    }
    ...
    call SRSNVTasks.TrainModel {
        input:
            ...
            jobMemory = memory_gb_TrainModel,
            monitoring_script = monitoring_script
    }
  ...
  if (snv_qualities_can_be_assigned && (!use_pre_trained_model)) {
    ...
    Array[File] model_files_ = flatten([select_first([TrainModel.model_files])])
  ...
  if (create_md5_checksum_outputs) {
        ...
        Array[File] output_files = select_all(
            [
              featuremap_output,
              featuremap_index_output,
              featuremap_df_output
            ])
