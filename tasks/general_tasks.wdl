...
task ExtractSorterStatsMetrics {
    input{
      ...
      String docker
      File monitoring_script
      String modules = "apptainer/1.4.5 jq/1.8.1"
      Int jobMemory = 2
      Int timeout = 2
    }

    parameter_meta {
      sorter_json_stats_files: "Array of json stat files"
    ...
    command <<<
        ...
        for stats_file in ~{sep=" " sorter_json_stats_files}; do
            ...
            MEAN_COV_TMP=$(mktemp)
            INPUT=$(readlink -f "$stats_file")
            INPUTDIR=$(dirname "$INPUT")
            apptainer exec \
              --bind ${PWD}:${PWD},${INPUTDIR}:${INPUTDIR},${TMPDIR}:${TMPDIR} \
              --userns \
              ~{docker} \
            sorter_stats_to_mean_coverage \
            --sorter-stats-json "$INPUT" \
            ...
            TOTAL_BASES=$(jq -re '.total_aligned_bases // .total_bases // error("missing total_aligned_bases and total_bases")' "$INPUT")
